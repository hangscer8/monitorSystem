...
lazy val front = project.settings( unmanagedSourceDirectories in Compile <++= (unmanagedSourceDirectories in shared) in Compile ).dependsOn(shared) //共享的库，需要被js使用，因此加上该句话
