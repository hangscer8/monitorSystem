...
trait IndexServiceTrait extends UtilTrait {
  ...
  def isAgentHealth(agentId: String): Boolean = {
    ...
    val queryAction1 = alarmEvents.filter(_.created > targetTime)
      ...
      .join(agentMachines).on(_._2.agentId === _.agentId).filter(_._2.agentId === agentId).result
    //一分钟内有告警数据则可以任务该agent不健康
    val queryAction2 = agentMachines.filter(_.isOnLine === 0).filter(_.agentId === agentId).map(_.agentId).result
    //或者离线的agent
