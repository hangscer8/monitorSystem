...
package ExtractUtils {
  ...
  object mac {
    ...
    def extractLoadAvg(str:String)={ // 2
      val Extract="""Load Avg:\s+(\d+.\d+),\s+(\d+.\d+),\s+(\d+.\d+)\s*""".r
    ...
    def extractCPUUsage(str:String)={ //3
      val Extract="""CPU usage:\s+(\d+.\d+)%\s+user,\s+(\d+.\d+)%\s+sys,\s+(\d+.\d+)%\s+idle\s*""".r
    ...
    def extractPhysMem(str:String)={ //drop 6
      val Extract="""PhysMem:\s+(\d+)M\s+used\s+\((\d+)M\s+wired\),\s+(\d+)M\s+unused.\s*""".r
